...
async function fetchArt() {
    showLoading();
    try {
        // Fetch 50 items to make filtering meaningful
        const response = await fetch(`${API_URL}?apikey=${API_KEY}&size=50&hasimage=1&sort=random`);
        const data = await response.json();
        
        // Filter out items without images and store
